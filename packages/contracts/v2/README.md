...
- TypeScript：`apps/web/src/lib/contracts/search-agent-v2.ts` 的 `contractErrorCodes`
- Python：`packages/contracts/python/search_agent_v2.py` 的 `CONTRACT_ERROR_CODES`
...
v2 合同位于独立目录，生产代码没有导入。回滚只需撤销 `packages/contracts/`、TypeScript 契约消费者测试和 Ajv 开发依赖，不涉及数据库、用户数据或 v1 运行行为。
