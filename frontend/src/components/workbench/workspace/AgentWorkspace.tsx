...
function PlanView({ steps }: { steps: AgentThreadState["plan"] }) {
  return <div className="scrollbar-subtle h-full overflow-y-auto px-5 py-5">{steps.map((step, index) => <div key={step.id} className="relative grid min-h-[68px] grid-cols-[20px_minmax(0,1fr)] gap-3" aria-label={`${step.title}，${step.status === "done" ? "已完成" : step.status === "in_progress" ? "执行中" : step.status === "blocked" ? "已阻塞" : "待执行"}`}>
    ...
    <div className={cn("min-w-0 pb-5 text-[16px] leading-6", step.status === "todo" ? "text-tertiary" : step.status === "blocked" ? "text-danger" : "text-ink")}><div>{step.title}</div>{step.notes ? <div className={cn("mt-1 text-[14px] leading-5", step.status === "blocked" ? "text-danger" : "text-secondary")}>{step.notes}</div> : null}</div>
  </div>)}</div>;
}
