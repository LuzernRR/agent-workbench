...
func InitMCPServer(appServer *AppServer) *mcp.Server {
	...
	// 项目部署只注册 Search Agent 实际需要的只读工具。上游写工具实现保留用于
	// 可审计升级对比，但不会出现在 tools/list，也没有对应 HTTP 路由。
		...
		defer func() {
			if r := recover(); r != nil {
				...
				result = &mcp.CallToolResult{
					Content: []mcp.Content{
						&mcp.TextContent{
							Text: fmt.Sprintf("工具 %s 执行时发生内部错误，请稍后重试。", toolName),
						},
					},
					IsError: true,
				}
...
// registerReadOnlyTools 只暴露项目允许的五类读取能力。
func registerReadOnlyTools(server *mcp.Server, appServer *AppServer) {
	...
	mcp.AddTool(server,
		&mcp.Tool{
			...
			Description: "读取指定小红书笔记的正文和作者信息，不加载全部评论",
			Annotations: &mcp.ToolAnnotations{
	...
	mcp.AddTool(server,
		...
		withPanicRecovery("user_profile", func(ctx context.Context, req *mcp.CallToolRequest, args UserProfileArgs) (*mcp.CallToolResult, any, error) {
			result := appServer.handleUserProfile(ctx, map[string]interface{}{
				"user_id":    args.UserID,
				"xsec_token": args.XsecToken,
			})
			return convertToMCPResult(result), nil, nil
	...
	logrus.Infof("Registered %d read-only MCP tools", 5)
}
