...
PROMPT_VERSION = "2026-07-31.v17-required-channel-evidence"
...
逐条判断输入中“当前轮已读取来源”是否直接支持用户当前问题且满足用户的筛选条件。未读候选绝对不能进入该字段。只有应展示的来源才令 include_in_details=true，并给出一条自然、具体、有信息量的中文 text；不相关、不适用、已过期、仅作反例或不满足用户筛选条件的来源必须令 include_in_details=false 且 text 为空。URL 必须原样复制。
...
输入只包含已经通过正文质量检查的 Evidence。逐条判断它是否直接支持用户当前问题且
符合用户筛选条件；只有应展示的来源令 include_in_details=true，并输出一条自然、具体、
有信息量的中文说明。不相关、不适用、已过期、仅作反例或不符合筛选条件的来源令
