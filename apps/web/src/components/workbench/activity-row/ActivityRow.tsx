...
export function SearchActivitySummary({ items, isCurrentStep = false }: { items: readonly ToolItem[]; isCurrentStep?: boolean }) {
  ...
  >
    ...
    {expanded ? <div className="ml-2 mt-1 space-y-1 border-l border-line pl-4 text-secondary" data-search-activity-details>
      {items.filter((item) => !["preparing", "running", "waiting"].includes(item.status) && item.settlementSummary).map((item) => <p key={`settled:${item.toolCallId}`} className="break-words" data-search-settlement>
        {item.query ? `${item.query}：` : ""}{item.outcomeStatus === "degraded" ? "受控降级，" : ""}{item.settlementSummary}
      </p>)}
        ...
        <div className="ml-2 mt-1 grid gap-x-8 gap-y-2 border-l border-line py-1.5 pl-5 text-[15px] leading-6 text-secondary md:grid-cols-2">
          ...
          {duration
            ...
            : ["preparing", "running", "waiting"].includes(item.status)
              ...
              : null}
          {item.error ? <Detail label={item.status === "unknown" ? "状态说明" : "错误信息"} danger={item.status !== "unknown"}>{item.status === "unknown" ? "结果状态未知，系统未自动重试" : getRunFailureMessage(item.error)}</Detail> : null}
