...
describe("Search Agent v1 白名单投影", () => {
  ...
  it("只持久化安全搜索字段，保留可点击来源且丢弃 snippet", () => {
    const projection = mapSearchAgentEvent(source({
      ...
      resultCount: 1,
      evidenceCount: 1,
      results: [{ title: "LangGraph", url: "https://docs.langchain.com/oss/python/langgraph", snippet: "不进入持久事件的候选摘要", verified: true }],
      cached: false
    }));
    expect(projection.events).toEqual([{ type: "tool.completed", payload: expect.objectContaining({
      ...
      resultCount: 1,
      evidenceCount: 1,
      sources: [{ title: "LangGraph", url: "https://docs.langchain.com/oss/python/langgraph", verified: true }],
  ...
  it("诚实映射 partial、failed、unknown 与 stopped", () => {
    ...
    const unknown = mapSearchAgentEvent(source({ type: "tool.unknown", toolCallId: "call_unknown", toolName: "web_search", query: "查询", reasonCode: "OUTCOME_UNKNOWN" }));
  ...
  it("把 unknown_tool 显示为被阻止的固定安全工具行", () => {
    const started = mapSearchAgentEvent(source({ type: "tool.started", toolCallId: "call_unknown", toolName: "unknown_tool", query: "原计划查询", cached: false }));
    const failed = mapSearchAgentEvent(source({ type: "tool.failed", toolCallId: "call_unknown", toolName: "unknown_tool", query: "原计划查询", provider: "none", reasonCode: "UNKNOWN_TOOL", message: "Researcher 请求了未注册工具", retryable: false }));
